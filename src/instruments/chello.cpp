...
Instrumentchello::Instrumentchello(const std::string &param) 
...
if ((file_name = kv("nombre_del_fichero")) == kv_null) {
...
void Instrumentchello::command(long cmd, long note, long vel) {
...
const vector<float> & Instrumentchello::synthesize() {
  if (not adsr.active()) {
    x.assign(x.size(), 0);
    bActive = false;
    return x;
