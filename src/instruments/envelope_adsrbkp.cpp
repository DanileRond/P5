...
namespace upc {
  ...
  EnvelopeADSR::EnvelopeADSR(unsigned int samplingRate, const string &param) : 
    rate(samplingRate) {
  ...
  int EnvelopeADSR::set(const string &param) {
    KeyValue kv(param);

    float valf;
    if (kv.to_float("ADSR_A", valf))  t_attack = valf;
    if (kv.to_float("ADSR_D", valf))  t_decay = valf;
    if (kv.to_float("ADSR_S", valf))  a_sustain = valf;
    if (kv.to_float("ADSR_R", valf))  t_release = valf;
    if (kv.to_float("ADSR_FS", valf)) factor_slope = valf;
